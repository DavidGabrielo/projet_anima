...
class Model extends Database
{
    ...
    {
        $slctRedondance = $this->getconnexion()->prepare("SELECT * FROM matieres WHERE matieres = :matiere AND niveau = :niveau AND categorie = 2");
    ...
    {
        return $this->getconnexion()->query("SELECT * FROM matieres WHERE niveau = $id AND categorie = 2 ORDER BY id")->fetchAll(PDO::FETCH_OBJ);
    }
    ...
    public function lectureSansId()
    {
        ...
        if (count($tabNiveau) > 0) {
            ...
            return $this->getconnexion()->query("SELECT * FROM matieres WHERE niveau = $idNiveau AND categorie = 2 ORDER BY id")->fetchAll(PDO::FETCH_OBJ);
        }
    }
